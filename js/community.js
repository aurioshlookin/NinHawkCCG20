...
window.getUserMedals = (inventory) => {
  ...
  // Medalha SS: tem pelo menos 1 carta SS
  if (tierCounts.SS >= 1) medals.push(`<span title="Possui carta SS" class="text-xs">💀</span>`);

  // Medalha S: tem pelo menos 1 carta S
  if (tierCounts.S >= 1) medals.push(`<span title="Possui carta S" class="text-xs">⭐</span>`);

  // Medalha Colecionador A: tem todas as cartas A
  if (tierTotals.A > 0 && tierCounts.A >= tierTotals.A) medals.push(`<span title="Coleção A completa" class="text-xs">💜</span>`);

  // Medalha Colecionador completo: tem todas as cartas do jogo
  const totalCards = window.cardDatabase.length;
  const totalOwned = Object.values(tierCounts).reduce((a, b) => a + b, 0);
  if (totalCards > 0 && totalOwned >= totalCards) medals.push(`<span title="Coleção completa!" class="text-xs">🏆</span>`);
