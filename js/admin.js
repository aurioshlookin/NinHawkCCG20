...
// ============================================================
// ÁREA DE RISCO: RESET DE CONTAS, BACKUPS E BANS DE IP
// ============================================================

window.resetAccountWithBackup = async () => {
  const uid = document.getElementById("adminTargetUID")?.value.trim();
  if (!uid) return window.showMessage("Por favor, insira o UID do jogador alvo.");
  
  const confirmReset = confirm("TEM CERTEZA? Isso criará um backup e removerá todas as cartas, progresso, fusões e moedas da conta desse jogador.");
  ...
  try {
    ...
    if (!userSnap.exists()) {
      return window.showMessage("Usuário não encontrado no banco de dados.");
    }

    const userData = userSnap.data();
    const timestamp = new Date().toISOString();

    // 1. Criar o Backup na coleção 'backups_accounts'
    const backupId = `${uid}_${timestamp}`;
    const backupRef = window.doc(window.db, "backups_accounts", backupId);
    
    await window.setDoc(backupRef, {
      ...userData,
      _backupMetadata: {
        originalUid: uid,
        date: timestamp,
        reason: "Admin Manual Reset",
        adminUid: window.currentUser ? window.currentUser.uid : "unknown"
      }
    });

    // 2. Aplicar o Estado Inicial (Vazio/Novo) mantendo as informações base
    const defaultState = {
      uid: uid,
      displayName: userData.displayName || "Jogador",
      email: userData.email || "",
      photoURL: userData.photoURL || "",
      role: userData.role || "player", // Mantém o papel para não tirar admin sem querer
      pullsAvailable: 5,
      premiumPullsAvailable: 0,
      inventory: {},
      tradesToday: 0,
      lastTradeDate: "",
      ...
      claimedAchievements: {},
      notifications: [{
        title: "Sua conta foi redefinida",
        desc: "Um administrador zerou o seu progresso. Seus dados antigos foram salvos em um backup de segurança.",
        date: new Date().toISOString(),
        read: false
      }],
      createdAt: userData.createdAt || window.serverTimestamp()
    };

    await window.setDoc(userRef, defaultState);
    
    window.showMessage(`Conta zerada com sucesso! Backup salvo como ID: ${backupId}`);
    if(window.loadAdminPlayersLog) window.loadAdminPlayersLog();

  } catch (error) {
    console.error("Erro ao resetar conta:", error);
    window.showMessage("Erro ao processar o reset: " + error.message);
  }
};

window.restoreAccountBackup = async () => {
  const uid = document.getElementById("adminTargetUID")?.value.trim();
  if (!uid) return window.showMessage("Insira o UID do jogador no campo para procurar os backups.");

  try {
    const backupsRef = window.collection(window.db, "backups_accounts");
    const querySnapshot = await window.getDocs(backupsRef);
    ...
    if (userBackups.length === 0) {
      return window.showMessage("Nenhum backup encontrado para este UID no sistema.");
    }

    // Ordenar do mais recente para o mais antigo
    userBackups.sort((a,b) => b.data._backupMetadata.date.localeCompare(a.data._backupMetadata.date));

    // Montar uma lista no formato texto
    let backupListText = "Backups disponíveis (Digite o NÚMERO correspondente para restaurar):\n\n";
    userBackups.forEach((bkp, index) => {
      const dateObj = new Date(bkp.data._backupMetadata.date);
      const packs = bkp.data.totalPacksOpened || 0;
      backupListText += `${index + 1} - Data: ${dateObj.toLocaleString()} (Packs abertos: ${packs})\n`;
    });

    const choice = prompt(backupListText);
    if (!choice) return;
    const selectedIndex = parseInt(choice) - 1;

    if (isNaN(selectedIndex) || selectedIndex < 0 || selectedIndex >= userBackups.length) {
      return alert("Seleção inválida. A restauração foi cancelada.");
    }

    const confirmRestore = confirm("Atenção: Restaurar esse backup vai sobrescrever os dados atuais do jogador (incluindo cartas que ele tenha pegado recentemente). Tem certeza?");
    if (!confirmRestore) return;

    const selectedBackup = userBackups[selectedIndex];
    
    // Retirar a tag de metadado do backup antes de devolver para a pasta "users"
    const dataToRestore = { ...selectedBackup.data };
    delete dataToRestore._backupMetadata;

    const userRef = window.doc(window.db, "users", uid);
    await window.setDoc(userRef, dataToRestore);
    
    window.showMessage("Conta restaurada com sucesso a partir do backup antigo!");
    if(window.loadAdminPlayersLog) window.loadAdminPlayersLog();

  } catch (error) {
    console.error("Erro ao restaurar backup:", error);
    window.showMessage("Erro ao tentar restaurar: " + error.message);
  }
};

window.banIP = async () => {
  const ipAddress = document.getElementById("adminTargetIP")?.value.trim();
  if (!ipAddress) return window.showMessage("Por favor, digite o IP que você deseja banir.");

  const confirmBan = confirm(`Tem certeza que deseja BANIR o acesso deste IP: ${ipAddress}?`);
  if (!confirmBan) return;

  try {
    const safeIpId = ipAddress.replace(/\./g, '_'); 
    const ipRef = window.doc(window.db, "banned_ips", safeIpId);
    
    await window.setDoc(ipRef, {
      ip: ipAddress,
      bannedAt: window.serverTimestamp(),
      bannedBy: window.currentUser ? window.currentUser.uid : "admin"
    });
    
    window.showMessage(`O IP ${ipAddress} foi BANIDO com sucesso! Ele não conseguirá mais interagir com o jogo.`);
    document.getElementById("adminTargetIP").value = "";
  } catch (error) {
    console.error("Erro ao banir IP:", error);
    window.showMessage("Erro ao aplicar o ban: " + error.message);
  }
};

window.unbanIP = async () => {
  const ipAddress = document.getElementById("adminTargetIP")?.value.trim();
  if (!ipAddress) return window.showMessage("Por favor, digite o IP que deseja desbanir.");

  const confirmUnban = confirm(`Tem certeza que deseja REMOVER o ban do IP: ${ipAddress}?`);
  ...
  try {
    ...
    const docSnap = await window.getDoc(ipRef);
    if (!docSnap.exists()) {
      return window.showMessage("Este IP não consta na lista de banidos do servidor.");
    }

    await window.deleteDoc(ipRef);
    window.showMessage(`O IP ${ipAddress} foi desbanido (restaurado) com sucesso!`);
    document.getElementById("adminTargetIP").value = "";

  } catch (error) {
    console.error("Erro ao desbanir IP:", error);
    window.showMessage("Erro ao tentar desbanir: " + error.message);
  }
};
